...
describe("Daylight.getReflectionColor - space-delimited-hsl_", () => {
    // space-delimited-hsl_1
    it("1: 第1引数が空白文字区切りのHSLの色表現の場合は、調整した色の空白文字区切りのHSL表現が返却される", () => {
        // テストの準備
        const delimiters = [ " ", "  ", "   " ];
        const hue = 210;
    ...
    it("2: 第1引数が空白文字区切りのHSLの色表現を含む場合は、調整した色の空白文字区切りのHSL表現に置換した内容が返却される", () => {
        // テストの準備
        const delimiters = [ " ", "  ", "   " ];
        const hue = 210;
